...
export abstract class BaseEyes {
  ...
  hasTextForDom(parentDomClass: string, childDom:string, text: string) {
    cy.get(parentDomClass).find(childDom).should('contain', text);
    return this;
  }

  seeIfCheckBoxIsChecked(dom: string){
...
seesForText(text : string) {
  cy.contains(text)
  return this;
}

seeTextOnDomIndex(locatorName: string, locatorValue: string, index : number, text: string) {
  cy.get(`[${locatorName}="${locatorValue}"]`).eq(index).should('contains.text', text);
  return this;
}

seeTextOnDom(dom: string, parent:string, child:string, text: string) {
  cy.get(dom).find(parent).find(child).should('contains.text', text);
  return this;
}

seeUrlExtension(extension: string){
...
export class BaseHands {
  ...
  clickOnButtonContainText(text: string){
    cy.contains(`${text}`).click()
    return this;
  }

  containsClick(locator:String, text:String){
    cy.get(`${locator}`).contains(`${text}`).click();
    return this;
  }

  containsClickOnATextInDropdown(locator:String, text:String){
       this.containsClick(`${locator}`, `${text}`)
   }
  ...
  clickOnDom(parentDomClass: string, childDom:string) {
    cy.get(parentDomClass).find(childDom).click();
    return this;
  }
}
